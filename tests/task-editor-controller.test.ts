...
describe('TaskEditorController', () => {
  ...
  describe('save', () => {
    ...
    it('does not reopen a recurring task when it is canceled', async () => {
      const { controller, plugin } = createHarness(['TODO Task text']);
      const repeat = {
        type: '+' as const,
        unit: 'd' as const,
    ...
    it('does not reopen a recurring task when it is CANCELLED', async () => {
      const { controller, plugin } = createHarness(['TODO Task text']);
      const repeat = {
        type: '+' as const,
        unit: 'd' as const,
    ...
    it('does not reopen a canceled task when only its schedule changes', async () => {
      const { controller, plugin } = createHarness(['CANCELED Task text']);
      const repeat = {
        type: '+' as const,
        unit: 'd' as const,
        value: 1,
    ...
    it('rolls a canceled recurring task forward when it is completed', async () => {
      const { controller, plugin } = createHarness(['CANCELED Task text']);
      const repeat = {
        type: '+' as const,
        unit: 'd' as const,
