...
- The dedicated Task List view gained sort direction and grouping. A ↑/↓ toggle beside the sort dropdown reverses the order (and resets to the method's natural direction when the method changes); a new Group by dropdown splits the list into counted sections (`folder`, `file`, `heading`, `status`, `priority`, `scheduled`/`deadline`/`closed`/`started` dates, or `tag`, with multi-tagged tasks listed under each) with its own ↑/↓ direction toggle. `tag` is also available as a sort method, grouped lists keep incremental loading, and saved searches capture the grouping and directions.
- Fixed the Task List search field's "Search options" / saved-searches dropdown not opening reliably. The dropdown modules are now loaded statically (no async import), the field is wired synchronously, and the panel opens on click as well as focus. Also added `description:`, `heading:` and `started:` to the options list.
...
- Recurring completions are now recorded in a collapsed `[!repeats]` callout at the end of the task, listing the iteration number, the closed time and the occurrence date, newest first. New settings `Track repeat history` (on by default) and `Repeat history limit` (default 50) control the log; the running total continues counting past the cap. Table-cell tasks skip the log, and the date picker now hides the repeat option for them because recurrence is not supported in cells.
...
- Date and time display (task list, tooltips, date picker and task editor) now follows a single locale source based on Obsidian's language, including localized relative labels and month/weekday names. English wording for near-future dates changed from "N days from now" to "In N days". Date text written into notes stays in the canonical org-mode format (`<YYYY-MM-DD Ddd>`, `[YYYY-MM-DD Ddd HH:mm]`) so notes remain portable across devices and locales.
...
- Added a `group-by:` option to embedded task lists that splits the results into labelled sections with a task count per section. Extended the query options:
  - `sort:` accepts an explicit direction (`asc`/`desc`) and an optional second key after a comma, e.g. `sort: priority desc, scheduled`. Tasks missing the sort value always sort last.
  - `group-by:` fields are `folder`, `file`, `heading` (alphabetical), `status`, `priority`, `scheduled`, `deadline`, `closed`, `started`, and `tag`, with an optional `asc`/`desc`. Value fields order naturally (priority High→Low, dates chronologically, `closed`/`started` most-recent-first, tags alphabetically); a `tag` places a task in several sections; a missing-value group comes last.
  - New `description:` and `heading:` search prefixes, usable in the search field and in embedded code blocks.
- Documented the embedded `show-urgency:` option.
